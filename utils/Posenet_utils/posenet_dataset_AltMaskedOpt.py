...
class LineModPoseDataset_AltMasked(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        depth_full_tensor = torch.from_numpy(d_img).unsqueeze(0).float()
        ...
        # --- NOVITÀ: Prepara l'immagine FULL per YOLO ---
        # YOLOv8 accetta tensori [C, H, W] con valori 0-255 (uint8) o 0-1 (float)
        # Convertiamo in tensore leggero (senza normalizzazione ImageNet)
        img_full_tensor = torch.from_numpy(img).permute(2, 0, 1).float() / 255.0 
        # Risultato: [3, 480, 640], valori 0.0-1.0
        ...
        return {
            'image': img_tensor,
            'image_full': img_full_tensor,
            'depth_full':depth_full_tensor 
            'depth': depth_tensor,
            'mask': mask_tensor, 
