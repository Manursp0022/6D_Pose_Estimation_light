...
class LineModPoseDataset(Dataset):
    def __init__(self, split_file, dataset_root, mode='train', img_size=224):
        ...
        # Resolve split file path. Accept absolute, relative, filename, or empty -> use autosplit_*.txt
        if not split_file:
            split_file = 'autosplit_' + ('train_ALL.txt' if mode == 'train' else 'val_ALL.txt')

        resolved_split = split_file
        # If split_file begins with a leading slash (e.g. '/dataset/...'), strip it for relative resolution
        if isinstance(resolved_split, str) and resolved_split.startswith(os.sep):
            resolved_split = resolved_split.lstrip(os.sep)

        # If the provided path is not an absolute file, try candidates under dataset_root
        if not os.path.isabs(resolved_split) or not os.path.exists(resolved_split):
            # Try relative to dataset_root
            candidates = [
                os.path.join(self.dataset_root, 'autosplit_' + ('train_ALL.txt' if mode=='train' else 'val_ALL.txt')),
            ]
            found = False
            for c in candidates:
                if c and os.path.exists(c):
                    ...
                    found = True
                    break
            if not found and not os.path.exists(resolved_split):
                raise FileNotFoundError(f"Split file not found: tried '{split_file}' and candidates under dataset_root.\nTried: {candidates}")

        with open(resolved_split, 'r') as f:
        ...
        # Helper: try to resolve image paths coming from split list
        def resolve_image_path(img_path):
            # If path starts with leading slash (common in provided split files), strip it
            if isinstance(img_path, str) and img_path.startswith(os.sep):
                img_path = img_path.lstrip(os.sep)

            if os.path.isabs(img_path) and os.path.exists(img_path):
                return img_path
            # Some split files may contain paths relative to different roots. Try several candidates.
            candidates = [
                os.path.join(self.dataset_root, img_path),
                os.path.join(self.dataset_root, 'Linemod_preprocessed', img_path),
                os.path.join(self.dataset_root, 'Linemod_preprocessed', 'Linemod_preprocessed', img_path),
                os.path.join(self.dataset_root, 'Linemod_preprocessed', 'Linemod_preprocessed', 'data', img_path),
                os.path.join(self.dataset_root, 'data', img_path),
            ]
            for c in candidates:
                if c and os.path.exists(c):
                    return c
            # As a last resort, return original (caller will handle missing files)
            return img_path

        for img_path_abs in tqdm(image_paths_raw):
            img_path_abs = resolve_image_path(img_path_abs)
            # Parsing del path
            ...
            # If path still not exists, try dataset-root-based construction
            if not os.path.exists(img_path_abs):
                candidate = os.path.join(self.dataset_root, 'data', folder_id, 'rgb', img_name)
            ...
            if folder_id not in loaded_gts:
                ...
                if os.path.exists(gt_path) and os.path.exists(cam_info_path):
                    ...
                    # Try alternate nested Linemod locations for gt/info
                    alt_gt = os.path.join(self.dataset_root, 'Linemod_preprocessed', 'Linemod_preprocessed', 'data', folder_id, 'gt.yml')
                    alt_info = os.path.join(self.dataset_root, 'Linemod_preprocessed', 'Linemod_preprocessed', 'data', folder_id, 'info.yml')
                    if os.path.exists(alt_gt) and os.path.exists(alt_info):
                        with open(alt_gt, 'r') as f:
                            loaded_gts[folder_id] = yaml.safe_load(f)
                        with open(alt_info, 'r') as c:
                            cam_infos_cache[folder_id] = yaml.safe_load(c)
                    else:
            ...
            #print(f"[DEBUG] Folder ID: {folder_id}, Image ID: {img_id_num}")
