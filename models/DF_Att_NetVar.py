...
from utils.Posenet_utils.CrossModalAttentionBBoxGuided import CrossModalAttentionBBoxGuided

class DenseFusion_Att_NetVar(nn.Module):
    def __init__(self, pretrained=True, temperature=2.0):
        ...
        self.cross_attention = CrossModalAttentionBBoxGuided(channels=512, feature_size=7)
    ...
    def _forward_fusion(self, rgb, bb_info, depth, mask=None, return_debug=False):
        if mask is not None:
            rgb = rgb * mask
            depth = depth * mask
        ...
        rgb_clean, depth_clean, spatial_mask = self.cross_attention(rgb_feat, depth_feat, bb_info)
        
        cat_feat = torch.cat([rgb_clean, depth_clean], dim=1) # [B, 1024, 7, 7]
        
        x = self.fusion_entry(cat_feat)
    ...
    def forward(self, rgb, depth, bb_info, cam_params, mask=None, return_debug=False):
        bs = rgb.size(0)
        fused_feat, rgb_enhanced, depth_enhanced, spatial_mask, dbg = self._forward_fusion(rgb, bb_info, depth, mask, return_debug)
