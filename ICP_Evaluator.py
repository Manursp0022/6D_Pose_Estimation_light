...
class ICPEvaluator:
    ...
    FIXED to pass bbox_norm and cam_params to ICP refiner.
    """
    ...
    def _setup_icp(self):
        """Setup ICP refiner - using the CROPPED version."""
        ...
        refiner = ICPRefiner(
            ...
            cam_K=cam_K,
            max_correspondence_distance=self.cfg.get('icp_max_dist', 0.05),
    ...
    def _get_depth_from_batch(self, depth_tensor):
        """Extract depth - already in meters from dataset."""
        ...
        for batch in tqdm(self.val_loader, desc="Evaluating"):
            ...
            for i in range(batch_size):
                ...
                if use_icp:
                    ...
                    # Get bbox_norm and cam_params for this sample
                    bbox_norm_i = bb_info[i].cpu().numpy()
                    cam_params_i = cam_params[i].cpu().numpy()
                    
                    # Run ICP with bbox and camera info
                    refined_R, refined_t, success = self.icp_refiner.refine(
                        pred_R, pred_t, depth, mask, obj_id,
                        bbox_norm=bbox_norm_i,
                        cam_params_norm=cam_params_i,
                        min_points=self.cfg.get('icp_min_points', 200)
                    )
                ...
                if is_correct:
                    total_correct += 1
                total_predictions += 1
                all_errors.append(add_error * 100)
...
if __name__ == "__main__":
    print("="*60)
    print("ICP Refinement Evaluation - FIXED")
    print("="*60)
    ...
    config = {
        ...
        # ICP - more lenient parameters
        'icp_max_dist': 0.05,       # 5cm - more lenient
        'icp_iterations': 50,
        'icp_n_points': 1000,
        'icp_min_points': 200,      # Lower threshold
