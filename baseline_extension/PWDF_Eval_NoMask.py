...
class DAMF_Evaluator:
    ...
    def _setup_model(self):
        ...
        #weights_path = os.path.join(self.cfg['model_dir'], 'DenseFusion_NetVar_NoMaskNoAtt.pth')
        ...
        # Controllo se è un checkpoint con metadati o solo state_dict
        ...
        # Removing prefx '_orig_mod.' , due to (torch.compile)
    ...
    def _remove_compile_prefix(self, state_dict):
        """
        Removes the prefix ‘_orig_mod.’ from weights saved with torch.compile().
        """
        new_state_dict = {}
    ...
    def _quaternion_to_matrix(self, quats):
        ...
        #our model output [w, x, y, z], scipy wants [x, y, z, w]
    ...
    def _process_yolo_batch_wIoU(self, rgb_paths, depth_paths, class_ids):
        ...
        if len(valid_indices) == 0:
            return None, None, None, None, [], [] #Empty list for iou
        
        rgb_batch = torch.stack(rgb_list)
        depth_batch = torch.stack(depth_list)
        bbox_batch = torch.stack(bbox_list)
        
        return rgb_batch, depth_batch, bbox_batch, valid_indices 
    ...
    def run(self):
        ...
        """
        print("\n" + "="*70)
        print(" STARTING EVALUATION - PWDF_Net")
        print("="*70)
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc="Evaluating"):
                ...
                # updating yolo missed for class
                for i in range(batch_size):
                ...
                for i in range(B):
                    ...
                    if obj_id not in self.models_3d:
                        print(f" Skipping obj_id={obj_id} (no 3D model)")
                        continue
                    
                    pts_3d = self.models_3d[obj_id]  # [N, 3] in metri
                    
                    # Compute ADD (o ADD-S for simmetric)
                    ...
                    all_trans_errors.append(t_err * 100.0)      
                    all_rot_errors_m.append(r_err_m * 100.0)    
